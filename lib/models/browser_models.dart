...
import 'package:web_browser/webview_tab.dart';
...
class BrowserModel extends ChangeNotifier {
  ...
  bool containsFavorite(FavoriteModel favorite) {
    return _favorites.contains(favorite) ||
        _favorites.firstWhere((element) => element.url == favorite.url,
                orElse: () => null) !=
            null;
  }
  ...
  void removeFavorite(FavoriteModel favorite) {
    if (!_favorites.remove(favorite)) {
      var favToRemove = _favorites.firstWhere(
  ...
  Future<void> restore() async {
    ...
    Map<String, WebArchiveModel> webArchives = webArchivesMap.map((key,
            value) =>
