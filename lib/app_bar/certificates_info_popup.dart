...
class _CertificateInfoPopupState extends State<CertificateInfoPopup> {
                    ...
                    children: <Widget>[
                      ...
                      RichText(
                        text: TextSpan(
                            ...
                            style:
                                TextStyle(color: Colors.purple, fontSize: 12.0),
  ...
  List<Widget> _buildKeyUsageSection(X509Certificate x509certificate) {
    ...
    var keyUsageIsCritical = criticalExtensionOIDs.firstWhere(
  ...
  List<Widget> _buildBasicConstraints(X509Certificate x509certificate) {
    ...
    var basicConstraintsIsCritical = criticalExtensionOIDs.firstWhere(
  ...
  List<Widget> _buildExtendedKeyUsage(X509Certificate x509certificate) {
    ...
    var extendedKeyUsageIsCritical = criticalExtensionOIDs.firstWhere(
                (oid) => oid == OID.extKeyUsage.toValue(),
                orElse: () => null) !=
            null
        ? "YES"
        : "NO";
  ...
  List<Widget> _buildSubjectKeyIdentifier(X509Certificate x509certificate) {
    ...
    var subjectKeyIdentifierIsCritical = criticalExtensionOIDs.firstWhere(
    ...
    if (subjectKeyIdentifier.length > 0) {
      var subjectKeyIdentifierToHexValue = subjectKeyIdentifier
          .map((byte) {
            var hexValue = byte.toRadixString(16);
            if (byte == 0 || hexValue.length == 1) {
              hexValue = "0" + hexValue;
  ...
  List<Widget> _buildAuthorityKeyIdentifier(X509Certificate x509certificate) {
    ...
    var authorityKeyIdentifierIsCritical = criticalExtensionOIDs.firstWhere(
    ...
    if (authorityKeyIdentifier.length > 0) {
      var authorityKeyIdentifierToHexValue = authorityKeyIdentifier
          .map((byte) {
            var hexValue = byte.toRadixString(16);
            if (byte == 0 || hexValue.length == 1) {
              hexValue = "0" + hexValue;
  ...
  List<Widget> _buildCertificatePolicies(X509Certificate x509certificate) {
    ...
    var certificatePoliciesIsCritical = criticalExtensionOIDs.firstWhere(
                (oid) => oid == OID.extKeyUsage.toValue(),
                orElse: () => null) !=
            null
        ? "YES"
  ...
  List<Widget> _buildCRLDistributionPoints(X509Certificate x509certificate) {
    ...
    var cRLDistributionPointsIsCritical = criticalExtensionOIDs.firstWhere(
  ...
  List<Widget> _buildAuthorityInfoAccess(X509Certificate x509certificate) {
    ...
    var authorityInfoAccessIsCritical = criticalExtensionOIDs.firstWhere(
  ...
  List<Widget> _buildSubjectAlternativeNames(X509Certificate x509certificate) {
    ...
    var subjectAlternativeNamesIsCritical = criticalExtensionOIDs.firstWhere(
